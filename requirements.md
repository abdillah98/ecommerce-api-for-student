...
Ecommerce API adalah platform backend berbasis RESTful API yang melayani transaksi e-commerce dasar seperti manajemen produk, kategori produk, keranjang belanja (cart), serta transaksi pembelian (checkout).
...
- Data seperti **Kategori**, **Produk**, **Keranjang**, dan **Pembelian** diisolasi menggunakan kunci asing (`project_id`). Pengguna dari Project A tidak dapat membaca atau memodifikasi data dari Project B.
...
Sistem ini mendefinisikan 7 tabel utama di dalam database SQLite:
...
### G. Purchase Items (`purchase_items`)
...
- **Request Body**:
  ```json
  {
    "projectTitle": "Toko Buku Online Kita",
    "projectDescription": "Backend e-commerce penjualan buku",
    "projectClass": "IF-22-A",
    "projectTeam": [
      { "name": "Budi Santoso", "nim": "22110001" },
      { "name": "Siti Rahma", "nim": "22110002" }
    ]
  }
  ```
...
- **Request Body**:
  ```json
  {
    ...
    "productDescription": "Laptop gaming performa tinggi",
    "productPrice": 15000000,
    "productStock": 10,
    "productImage": "https://example.com/rog.jpg"
  }
  ```
- **Response (201 Created)**:
  ```json
...
### 4.6 Modul Pembelian / Purchase & Checkout (Protected)

#### A. Checkout Keranjang (`POST /purchases`)
Melakukan transaksi checkout seluruh item di dalam cart saat ini.
- **Proses Sistem**:
  1. Validasi kecukupan stok masing-masing produk. Jika stok tidak cukup, transaksi dibatalkan (Rollback).
  2. Kalkulasi `totalPrice` otomatis berdasarkan harga aktual produk.
  3. Mengurangi stok produk yang dibeli.
  4. Menyalin item cart ke data transaksi (`purchase_items`).
  5. Mengosongkan cart user secara otomatis.
- **Request Body**: (Kosong, sistem memproses otomatis isi cart)
  ```json
  {}
  ```
- **Response (201 Created)**:
...
Mengembalikan invoice lengkap beserta daftar produk yang dibeli.
- **Response (200 OK)**:
  ```json
  {
    "success": true,
...
### Keamanan Data
1. **Autentikasi Token JWT**: Seluruh request ke modul Kategori, Produk, Cart, dan Purchase diverifikasi oleh `authMiddleware`. Informasi `projectId` dan `userId` diekstrak dari JWT, mencegah manipulasi payload request body untuk mengakses tenant lain.
...
- `400 Bad Request`: Input tidak valid, stok produk tidak cukup.
- `401 Unauthorized`: Token JWT tidak valid atau tidak disertakan.
- `404 Not Found`: Produk, kategori, atau transaksi tidak ditemukan.
